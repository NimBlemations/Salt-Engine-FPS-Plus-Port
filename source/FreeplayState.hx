...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (StoryMenuState.weekUnlocked[2] || isDebug)
			addWeek(['Daddys-Girl', 'Salty-Love', 'Daughter-Complex'], 1, ['dad']);

		if (StoryMenuState.weekUnlocked[2] || isDebug)
			addWeek(['Sweet-n-Spooky', 'Sour-n-Scary', 'Opheebop'], 2, ['spooky', 'spooky', "monster"]);

		if (StoryMenuState.weekUnlocked[3] || isDebug)
			addWeek(['Protect', 'Defend', 'Safeguard'], 3, ['pico']);

		if (StoryMenuState.weekUnlocked[4] || isDebug)
			addWeek(['Indie-Star', 'Rising-Start', 'Superstar'], 4, ['mom']);
